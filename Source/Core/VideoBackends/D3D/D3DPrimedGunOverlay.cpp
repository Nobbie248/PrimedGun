...
{
  DestroyOverlaySwapchain();
  DestroyLaserSwapchain();
}

void D3DPrimedGunOverlay::DestroyOverlaySwapchain()
{
  auto& overlay = m_overlay_swapchain;
...
bool D3DPrimedGunOverlay::EnsureOverlaySwapchain(uint32_t content_kind, uint32_t generation,
...
bool D3DPrimedGunOverlay::AppendLayers(std::vector<XrCompositionLayerBaseHeader*>* layers)
{
  ...
  if (!EnsureOverlaySwapchain(content_kind, generation, width, height, pixels))
    return false;
  ...
  if (menu && left_pose.valid)
  {
    ...
    m_overlay_layer.pose.position = {left_pose.position.x + offset.x,
  ...
  if (menu && right_pose.valid && EnsureLaserSwapchain())
  {
    ...
    m_laser_layer.pose.orientation =
        PGO::MulQuat(q, {-0.70710678f, 0.0f, 0.0f, 0.70710678f});
    m_laser_layer.pose.position = {right_pose.position.x + forward.x * 0.40f,
