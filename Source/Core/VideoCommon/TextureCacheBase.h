...
public:
  ...
  // Returns the XXH64 hash of the texture bound to the given stage (0-7), or 0 if none bound.
  // Matches the hash in texture dump filenames (tex1_WxH_<THIS_HASH>_...).
  ...
  void CloseMetroidPrime1ThermalSourceWindow();
  bool HasMetroidPrime1ThermalStereoSource() const;
...
private:
  ...
  bool m_metroid_prime1_thermal_source_window = false;
  int m_metroid_prime1_thermal_source_window_priority = 0;
  RcTcacheEntry m_metroid_prime1_thermal_stereo_source;
