                                  ...
                                  const ShaderHostConfig& host_config, bool bounding_box)
{
  ...
  out.Write("\tint4 " I_COLORS "[4];\n"
            ...
            "\tuint  logic_op_mode;\n"
            "\tuint  time_ms;\n"
            "\tint   " I_VR_LAYER_OVERRIDE ";\n"
            "\tuint  cvr_layer_override_pad;\n"
            "}};\n\n");
...
uint WrapCoord(int coord, uint wrap, int size) {{
  ...
  {
    // VR draws can render to a stereo render target while many sampled game/effect textures are
    // still single-layer views. Clamp per sampled texture so stereo copies can use layer 1 while
    // ordinary single-layer textures safely stay on layer 0.
                       ...
                       const pixel_shader_uid_data* uid_data, ShaderCode& out)
{
  ...
  if (api_type == APIType::D3D && uid_data->genMode_numtexgens > 0)
  {
    out.Write("\tif (" I_VR_LAYER_OVERRIDE " == -2)\n");
    out.Write("\t  layer = int(round(frag_input.tex0.z));\n");
    out.Write("\telse if (" I_VR_LAYER_OVERRIDE " >= 0)\n");
    out.Write("\t  layer = " I_VR_LAYER_OVERRIDE ";\n");
  }
  else
  {
    out.Write("\tif (" I_VR_LAYER_OVERRIDE " >= 0)\n");
    out.Write("\t  layer = " I_VR_LAYER_OVERRIDE ";\n");
  }
