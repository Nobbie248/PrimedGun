...
public:
  ...
  void OnDraw();
  u32 GetDrawCounterForDebug() const { return m_draw_counter; }
...
private:
  ...
  // Per-frame counter for METROID_THERMAL_EFFECT_GUN draws. The classifier collapses both
  // the cold filter and the heat source into ThermalEffectGun, so this keeps later composite
  // passes pinned to layer 0 unless a known stereo thermal source is bound.
  int m_metroid_thermal_effect_gun_draws = 0;
