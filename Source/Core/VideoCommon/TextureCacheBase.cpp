...
static int MetroidPrime1ThermalSourceReasonPriority(std::string_view reason)
{
  if (reason == "METROID_THERMAL_HEAT_GEOMETRY")
    return 2;
  if (reason == "METROID_THERMAL_GUN_AND_DOOR")
    return 1;
  return 0;
}
...
{
  FlushEFBCopies();
  CloseMetroidPrime1ThermalSourceWindow();
  m_metroid_prime1_thermal_stereo_source.reset();
  TMEM::InvalidateAll();
...
u32 TextureCacheBase::GetBoundTextureNativeWidth(u32 stage) const
{
  if (stage >= m_bound_textures.size() || !m_bound_textures[stage])
    return 0;
  return m_bound_textures[stage]->native_width;
}

u32 TextureCacheBase::GetBoundTextureNativeHeight(u32 stage) const
{
  if (stage >= m_bound_textures.size() || !m_bound_textures[stage])
    return 0;
  return m_bound_textures[stage]->native_height;
}

void TextureCacheBase::BeginMetroidPrime1ThermalSourceWindow(std::string_view reason)
{
  const int priority = MetroidPrime1ThermalSourceReasonPriority(reason);
  if (m_metroid_prime1_thermal_source_window &&
      priority < m_metroid_prime1_thermal_source_window_priority)
    return;
  ...
  m_metroid_prime1_thermal_source_window = true;
  m_metroid_prime1_thermal_source_window_priority = priority;
}

void TextureCacheBase::CloseMetroidPrime1ThermalSourceWindow()
{
  m_metroid_prime1_thermal_source_window = false;
  m_metroid_prime1_thermal_source_window_priority = 0;
}

bool TextureCacheBase::HasMetroidPrime1ThermalStereoSource() const
{
  return m_metroid_prime1_thermal_stereo_source &&
         m_metroid_prime1_thermal_stereo_source->texture &&
         m_metroid_prime1_thermal_stereo_source->texture->GetLayers() >= 2;
}
                                           ...
                                           const TextureInfo& texture_info)
{
  ...
  std::string texture_name = texture_info.CalculateTextureName().GetFullName();
  ...
  if (entry)
  {
    ...
    if (m_metroid_prime1_thermal_source_window &&
        IsMetroidPrime1ThermalStereoSourceCandidate(entry->native_width, entry->native_height,
                                                    entry->GetNumLayers(), is_depth_copy,
                                                    is_xfb_copy))
    {
      m_metroid_prime1_thermal_stereo_source = entry;
    }
