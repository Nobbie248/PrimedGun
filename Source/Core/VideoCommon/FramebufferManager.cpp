...
bool FramebufferManager::IsEFBFoveated() const
{
  return m_efb_foveated.value_or(false) && m_efb_color_texture &&
         m_efb_color_texture->GetLayers() == 2 && m_efb_color_texture->GetSamples() == 1 &&
...
FramebufferState FramebufferManager::GetEFBFramebufferState() const
{
  ...
  ret.fragment_density_map = m_efb_framebuffer->HasFragmentDensityMap() ? 1u : 0u;
  return ret;
}
...
bool FramebufferManager::CreateEFBFramebuffer(int efb_scale)
{
  ...
  // Latched once per session: the EFB framebuffer and every GX pipeline must agree on
  // whether the render pass has a density map, and pipelines are cached across EFB
  // recreations. Only the density map *contents* (FoveationLevel) may change later.
