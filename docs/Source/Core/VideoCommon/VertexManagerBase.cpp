...
namespace
{
const char* HandlingToString(ShaderHunter::HandlingType handling)
{
  switch (handling)
  {
  ...
  if (!m_cull_all)
  {
    ...
    if (!skip)
    {
      ...
      if (m_current_pipeline_object)
      {
        ...
        // Per-draw default: no forced PS texture sampling layer.
        int forced_ps_layer = -1;
        bool forced_headlocked_full_view = false;
        float forced_headlocked_right_eye_x_offset = 0.0f;
        pixel_shader_manager.SetVRTextureLayerOverride(forced_ps_layer);
        ShaderHunter::HandlingType matched_handling = ShaderHunter::HandlingType::Skip;
        if (hunter.IsEnabled() || hunter.HasOverrides() || hunter.IsDebugLogging())
        {
          ...
          ShaderHunter::RuntimeElementSignature runtime_signature;
          runtime_signature.valid = true;
          runtime_signature.perspective = xfmem.projection.type == ProjectionType::Perspective;
          runtime_signature.use_projection = true;
          runtime_signature.use_layer = !runtime_signature.perspective;
          runtime_signature.use_viewport = true;
          runtime_signature.use_scissor = !runtime_signature.perspective;
          runtime_signature.use_render_state = true;
          runtime_signature.viewport_x = static_cast<int>(std::lround(xfmem.viewport.xOrig));
          runtime_signature.viewport_y = static_cast<int>(std::lround(xfmem.viewport.yOrig));
          runtime_signature.viewport_width = static_cast<int>(std::lround(xfmem.viewport.wd));
          runtime_signature.viewport_height = static_cast<int>(std::lround(xfmem.viewport.ht));
          runtime_signature.scissor_left = bpmem.scissorTL.x;
          runtime_signature.scissor_top = bpmem.scissorTL.y;
          runtime_signature.scissor_right = bpmem.scissorBR.x;
          runtime_signature.scissor_bottom = bpmem.scissorBR.y;
          runtime_signature.alpha_test_hex = bpmem.alpha_test.hex;
          runtime_signature.blend_color_update = bpmem.blendmode.color_update != 0;
          runtime_signature.blend_alpha_update = bpmem.blendmode.alpha_update != 0;
          runtime_signature.ztest = bpmem.zmode.test_enable != 0;
          runtime_signature.zupdate = bpmem.zmode.update_enable != 0;
          runtime_signature.zfunc = static_cast<int>(bpmem.zmode.func.Value());
          runtime_signature.ortho_layer = geometry_shader_manager.vr_ortho_draw_counter;

          const float* projection = xfmem.projection.rawProjection.data();
          if (runtime_signature.perspective)
          {
            ...
            const float zfar = projection[5] / projection[4];
            const float znear = zfar * projection[4] / (projection[4] - 1.0f);
            runtime_signature.perspective_hfov_x100 =
                static_cast<int>(std::lround(hfov * 100.0f));
            runtime_signature.perspective_vfov_x100 =
                static_cast<int>(std::lround(vfov * 100.0f));
            runtime_signature.perspective_near_x1000 =
                static_cast<int>(std::lround(znear * 1000.0f));
            runtime_signature.perspective_far_x100 =
                static_cast<int>(std::lround(zfar * 100.0f));
          }
          else
          {
            ...
            const float zfar = projection[5] / projection[4];
            const float znear = (1.0f + projection[4] * zfar) / projection[4];
            runtime_signature.ortho_left_x100 = static_cast<int>(std::lround(left * 100.0f));
            runtime_signature.ortho_right_x100 = static_cast<int>(std::lround(right * 100.0f));
            runtime_signature.ortho_top_x100 = static_cast<int>(std::lround(top * 100.0f));
            runtime_signature.ortho_bottom_x100 =
                static_cast<int>(std::lround(bottom * 100.0f));
            runtime_signature.ortho_near_x100 = static_cast<int>(std::lround(znear * 100.0f));
            runtime_signature.ortho_far_x100 = static_cast<int>(std::lround(zfar * 100.0f));
          }
          hunter.SetCurrentDrawSignature(runtime_signature);
          ...
          if (!hunter_skip)
          {
            matched_handling = hunter.GetOverrideHandling(vs_hash, ps_hash, gs_hash);
            if (matched_handling == ShaderHunter::HandlingType::Screen)
            {
              geometry_shader_manager.vr_stereo_override = -1.0f;
              // Manual layer from override (-1 = auto)
              const int manual_layer = hunter.GetOverrideLayer(vs_hash, ps_hash, gs_hash);
              if (manual_layer >= 0)
                geometry_shader_manager.vr_ortho_layer_override = manual_layer;
              const float ed = hunter.GetOverrideElementDepth(vs_hash, ps_hash, gs_hash);
              if (ed >= 0.0f)
                geometry_shader_manager.vr_element_depth_override = ed;
            }
            else if (matched_handling == ShaderHunter::HandlingType::Fullscreen)
            {
              geometry_shader_manager.vr_stereo_override = 0.0f;
            }
            else if (matched_handling == ShaderHunter::HandlingType::FullscreenMono)
            {
              // Hydra-style fullscreen mono:
              // keep the original clip-space geometry in both eyes and disable stereo reprojection,
              // while forcing mono texture-array sampling for the directly matched effect pass.
              geometry_shader_manager.vr_stereo_override = 0.0f;
              geometry_shader_manager.vr_head_locked_full_view_override = false;
              geometry_shader_manager.vr_head_locked_right_eye_x_offset_override = 0.0f;
              forced_headlocked_full_view = false;
              forced_headlocked_right_eye_x_offset = 0.0f;
              forced_ps_layer = 0;
              pixel_shader_manager.SetVRTextureLayerOverride(forced_ps_layer);
            }
            else if (matched_handling == ShaderHunter::HandlingType::HeadLocked)
            {
            ...
            else if (matched_handling == ShaderHunter::HandlingType::UnitsPerMeter)
            {
              const float upm = hunter.GetOverrideUnitsPerMeter(vs_hash, ps_hash, gs_hash);
              if (upm > 0.0f)
                geometry_shader_manager.vr_units_per_meter_override = upm;
            }
            else if (g_ActiveConfig.vr_metroid_visor_fix &&
                     m_has_recent_fullscreen_mono_draw &&
                     (m_draw_counter - m_recent_fullscreen_mono_draw) <= 6 &&
                     m_recent_fullscreen_mono_gs_hash == gs_hash &&
                     m_recent_fullscreen_mono_tex0 != 0 &&
                     m_recent_fullscreen_mono_tex0 == tex_hashes[0] &&
                     (m_recent_fullscreen_mono_tex1 == 0 ||
                      m_recent_fullscreen_mono_tex1 == tex_hashes[1]))
            {
              matched_handling = ShaderHunter::HandlingType::FullscreenMono;
              geometry_shader_manager.vr_stereo_override = 0.0f;
              geometry_shader_manager.vr_head_locked_full_view_override = false;
              geometry_shader_manager.vr_head_locked_right_eye_x_offset_override = 0.0f;
              forced_headlocked_full_view = false;
              ...
              INFO_LOG_FMT(
                  VIDEO,
                  "VR_MONO_CARRY #{} from #{}: VS={:08x} PS={:08x} GS={:08x} proj={}",
                  m_draw_counter, m_recent_fullscreen_mono_draw, vs_hash, ps_hash, gs_hash,
                  xfmem.projection.type == ProjectionType::Orthographic ? "ortho" : "persp");
            }
            else
            {
          ...
          // Thermal/fullscreen-mono targeted trace.
          // Always on so troubleshooting does not depend on Shader Hunter debug toggle.
          if (g_ActiveConfig.vr_metroid_visor_fix &&
              matched_handling == ShaderHunter::HandlingType::FullscreenMono)
          {
            const bool perspective = xfmem.projection.type == ProjectionType::Perspective;
            const bool ortho = xfmem.projection.type == ProjectionType::Orthographic;
            const float stereo_override = geometry_shader_manager.vr_stereo_override;
            const bool stereo_override_valid = !std::isnan(stereo_override);
            m_has_recent_fullscreen_mono_draw = true;
            m_recent_fullscreen_mono_draw = m_draw_counter;
            m_recent_fullscreen_mono_gs_hash = gs_hash;
            m_recent_fullscreen_mono_tex0 = tex_hashes[0];
            m_recent_fullscreen_mono_tex1 = tex_hashes[1];
            INFO_LOG_FMT(
                VIDEO,
                "VR_MONO_DBG #{}: VS={:08x} PS={:08x} GS={:08x} handling={} proj={} "
                "stereo_override_valid={} stereo_override={:.2f} ps_layer={} full_view={} "
                "right_eye_x_shift={:.2f} "
                "tex0={:016x} tex1={:016x} tex2={:016x} tex3={:016x}",
                m_draw_counter, vs_hash, ps_hash, gs_hash, HandlingToString(matched_handling),
                perspective ? "persp" : (ortho ? "ortho" : "other"), stereo_override_valid,
                stereo_override_valid ? stereo_override : 0.0f, forced_ps_layer,
                forced_headlocked_full_view, forced_headlocked_right_eye_x_offset,
                tex_hashes[0], tex_hashes[1], tex_hashes[2], tex_hashes[3]);
          }
          else if (m_has_recent_fullscreen_mono_draw &&
                   m_draw_counter > m_recent_fullscreen_mono_draw &&
                   (m_draw_counter - m_recent_fullscreen_mono_draw) <= 6 &&
                   xfmem.projection.type == ProjectionType::Orthographic)
          {
            INFO_LOG_FMT(
                VIDEO,
                "VR_MONO_NEAR #{}:+{} VS={:08x} PS={:08x} GS={:08x} handling={} "
                "alpha_test={:#x} blend(col={},alpha={}) zt={} zupd={} zf={} "
                "VP({:.0f},{:.0f} {:.0f}x{:.0f}) SC({},{} {},{}) "
                "tex0={:016x} tex1={:016x}",
                m_draw_counter, m_draw_counter - m_recent_fullscreen_mono_draw, vs_hash, ps_hash,
                gs_hash, HandlingToString(matched_handling), bpmem.alpha_test.hex,
                bpmem.blendmode.color_update != 0, bpmem.blendmode.alpha_update != 0,
                bpmem.zmode.test_enable != 0, bpmem.zmode.update_enable != 0, bpmem.zmode.func,
                xfmem.viewport.xOrig, xfmem.viewport.yOrig, xfmem.viewport.wd, xfmem.viewport.ht,
                bpmem.scissorTL.x, bpmem.scissorTL.y, bpmem.scissorBR.x, bpmem.scissorBR.y,
                tex_hashes[0], tex_hashes[1]);
          }
          ...
          if (hunter.IsDebugLogging()) [[unlikely]]
          {
            ...
            if (proj.type == ProjectionType::Orthographic)
            {
              ...
              INFO_LOG_FMT(VIDEO,
                           ...
                           "z={} zf={} | layer_ctr={} layer_ovr={} | skip={} handling={} "
                           "ps_layer={}",
                           ...
                           HandlingToString(matched_handling), forced_ps_layer);
            }
            else
            {
              const float* p = proj.rawProjection.data();
        ...
        if (!hunter_skip)
        {
          // Shader Hunter pink highlight: compile the pink PS lazily on first use.
...
void VertexManagerBase::OnEndFrame()
{
  ...
  m_draw_counter = 0;
  m_has_recent_fullscreen_mono_draw = false;
  m_recent_fullscreen_mono_draw = 0;
  m_recent_fullscreen_mono_gs_hash = 0;
  m_recent_fullscreen_mono_tex0 = 0;
  m_recent_fullscreen_mono_tex1 = 0;
