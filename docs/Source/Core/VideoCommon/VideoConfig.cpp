...
namespace
{
// Keep the global UPM slider conservative, but allow per-game/per-shader overrides beyond it.
constexpr float VR_UNITS_PER_METER_EFFECTIVE_MIN = 0.01f;
constexpr float VR_UNITS_PER_METER_EFFECTIVE_MAX = 1000.0f;
}
...
void VideoConfig::Refresh()
{
  ...
  vr_units_per_meter = std::clamp(Config::Get(Config::GFX_VR_UNITS_PER_METER),
                                  VR_UNITS_PER_METER_EFFECTIVE_MIN,
                                  VR_UNITS_PER_METER_EFFECTIVE_MAX);
  ...
  vr_head_locked_curvature =
      std::clamp(Config::Get(Config::GFX_VR_HEAD_LOCKED_CURVATURE),
