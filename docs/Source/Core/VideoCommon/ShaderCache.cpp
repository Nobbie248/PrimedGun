...
// Helper: check if a custom shader replacement file exists for the given UID hash.
// Returns true and fills out_source if found at Load/Shaders/<game_id>/<hash>-<suffix>.txt
static bool TryLoadCustomShader(const u8* uid_raw, size_t uid_size, const char* type_suffix,
                                std::string& out_source)
{
  if (!g_ActiveConfig.vr_load_custom_shaders)
    return false;
  ...
  const u32 hash = Common::ComputeCRC32(uid_raw, static_cast<u32>(uid_size));
  const std::string path = File::GetUserPath(D_LOAD_IDX) + "Shaders/" + game_id + "/" +
                           fmt::format("{:08x}-{}.txt", hash, type_suffix);

  if (!File::ReadFileToString(path, out_source))
    return false;

  INFO_LOG_FMT(VIDEO, "ShaderCache: Loaded custom {} shader {:08x} from '{}'", type_suffix, hash,
               path);
  return true;
}
...
const AbstractShader* ShaderCache::CreateGeometryShader(const GeometryShaderUid& uid)
{
  ...
  std::string custom_source;
  std::unique_ptr<AbstractShader> shader;
  if (TryLoadCustomShader(uid.GetUidDataRaw(), uid.GetUidDataSize(), "gs", custom_source))
  {
    shader = g_gfx->CreateShaderFromSource(ShaderStage::Geometry, custom_source, nullptr,
                                           fmt::format("Custom GS: {}", *uid.GetUidData()));
  }
  else
  {
    shader = g_gfx->CreateShaderFromSource(ShaderStage::Geometry, source_code.GetBuffer(), nullptr,
